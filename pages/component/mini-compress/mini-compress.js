      ...
      wx.yx.chooseImage = ({ count = 3, success = '', fail = '', imgWidth = 1080, maxh2w = 8}) => {
        ...
        wx.chooseImage({
          ...
          success(res) {
            ...
            let nonPictureFiles = [],
                pictureFiles = [];
            for(let i=0,l=tempFiles.length; i<l; i++){
              if(tempFiles[i].path.split(".").pop().toLowerCase() == "pdf" ||
                tempFiles[i].path.split(".").pop().toLowerCase() == "ofd"){
                  nonPictureFiles.push(tempFiles[i]);
              }else {
                pictureFiles.push(tempFiles[i]);
              }
            }
            _this.setData({
              nonPictureFiles,
            ...
            if(pictureFiles.length > 0){
              let newImgArray = _this._packageImgsLocally(pictureFiles)
      ...
      for (let i = 0; i < imgLen; i++) {
        wx.getImageInfo({
          ...
          success(res) {
            ...
            newImgArray = newImgArray.concat(_this.data.nonPictureFiles);
            if (hasCalNum === imgLen) {
